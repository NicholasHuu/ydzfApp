...
appMoudle.controller("cigaretteManagementCtrl", ["$scope", "$state","$stateParams","initData","$rootScope","appService","$ionicHistory",
 function($scope, $state,$stateParams,initData,$rootScope,appService,$ionicHistory) {
    ...
    $scope.$watch('$viewContentLoaded', function() {
        ...
        appService.conditionFlagChange($scope.cigaretteParams,property =>{
      ...
      */
    $scope.checkChange = (value,resultIdx,handle,inputVal) =>{
    ...
    $scope.cigaretteClick=function(){
        ...
        if(!cigaretteParams.me11Id){
            ...
            if (!cigaretteParams.meReslut16Detail && $scope.cigaretteParams.ME_RESLUT16){
                ...
                appService.requireForWMF(wmf, cigaretteParams, "RegieProExaminfoAdapter", "saveExamInfo03", (data) => {
                    $rootScope.marketRecordHasChange=false;
                    //责令整改与物品登记都未开放
